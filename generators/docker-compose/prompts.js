...
function askForPath() {
    ...
    this.prompt(prompts).then(function (props) {
        ...
        for(var i = this.appsFolders.length - 1; i >= 0; i--) {
            if (this.appsFolders[i] === 'jbooter-registry' || this.appsFolders[i] === 'registry') {
...
function askForApps() {
    ...
    this.prompt(prompts).then(function (props) {
        ...
        this.appsFolders.forEach(function (appFolder) {
            ...
            var config = fileData['generator-jbooter'];
...
function askForMonitoring() {
    ...
    var prompts = [{
        ...
        choices: [
            ...
            {
                value: 'elk',
                name: 'Yes, for logs and metrics with the JBooter Console (based on ELK)'
            },
            {
                value: 'prometheus',
                name: 'Yes, for metrics only with Prometheus (only compatible with JBooter >= v3.12)'
            }
        ],
        default: 'no'
    }];
...
function askForServiceDiscovery() {
    ...
    if(serviceDiscoveryEnabledApps.every(app => app.serviceDiscoveryType === 'consul')){
        ...
        this.log(chalk.green('JBooter registry detected as the service discovery and configuration provider used by your apps'));
        done();
    }
    else {
...
function askForAdminPassword() {
    ...
    var prompts = [{
        type: 'input',
        name: 'adminPassword',
        message: 'Enter the admin password used to secure the JBooter Registry',
        default: 'admin',
        validate: function (input) {
            if(input.length < 5) {
        ...
        if(file.isDirectory()) {
            if( (shelljs.test('-f', input + file.name + '/.yo-rc.json'))
                ...
                try {
                    ...
                    if ((fileData['generator-jbooter'].baseName !== undefined)
                        && ((composeApplicationType === undefined)
                            || (composeApplicationType === fileData['generator-jbooter'].applicationType)
                            || ((composeApplicationType === 'microservice') && ('gateway' === fileData['generator-jbooter'].applicationType))
                            || ((composeApplicationType === 'microservice') && ('uaa' === fileData['generator-jbooter'].applicationType)))) {
