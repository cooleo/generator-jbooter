...
module.exports = DockerComposeGenerator.extend({
    ...
    initializing: {
        sayHello: function() {
            this.log(chalk.white('Welcome to the JBooter Docker Compose Sub-Generator '));
        ...
        setupServerVars: function () {
            ...
            this.DOCKER_JBOOTER_REGISTRY = constants.DOCKER_JBOOTER_REGISTRY;
            this.DOCKER_JBOOTER_CONSOLE = constants.DOCKER_JBOOTER_CONSOLE;
            this.DOCKER_JBOOTER_ELASTICSEARCH = constants.DOCKER_JBOOTER_ELASTICSEARCH;
            this.DOCKER_JBOOTER_LOGSTASH = constants.DOCKER_JBOOTER_LOGSTASH;
        ...
        setAppsYaml: function() {
            ...
            this.appsFolders.forEach(function (appsFolder, index) {
                ...
                if (appConfig.applicationType === 'monolith' && this.monitoring === 'elk') {
                    yamlConfig.environment.push('JBOOTER_LOGGING_LOGSTASH_ENABLED=true');
                    yamlConfig.environment.push('JBOOTER_LOGGING_LOGSTASH_HOST=jbooter-logstash');
                    yamlConfig.environment.push('JBOOTER_METRICS_LOGS_ENABLED=true');
                    yamlConfig.environment.push('JBOOTER_METRICS_LOGS_REPORT_FREQUENCY=60');
                }

                if (this.monitoring === 'prometheus') {
                    yamlConfig.environment.push('JBOOTER_METRICS_PROMETHEUS_ENABLED=true');
                    yamlConfig.environment.push('JBOOTER_METRICS_PROMETHEUS_ENDPOINT=/prometheusMetrics');
                }

                if (this.serviceDiscoveryType === 'eureka') {
                    // Re-configure the JBooter Registry if it is already configured
                    ...
                    // Configure the JBooter Registry if it is not already configured
