...
module.exports = EntityGenerator.extend({
    ...
    initializing: {
        getConfig: function (args) {
            ...
            this.jbooterConfigDirectory = '.jbooter';
            this.mainClass = this.getMainClassName();
            this.microserviceAppName = '';

            this.filename = this.jbooterConfigDirectory + '/' + this.entityNameCapitalized + '.json';
        ...
        validateEntityName: function () {
            if (!(/^([a-zA-Z0-9_]*)$/.test(this.name))) {
                ...
                this.error(chalk.red('The entity name cannot contain a Java or JBooter reserved keyword'));
            }
        },
    ...
    _loadJson: function () {
        ...
        if (_.isUndefined(this.entityTableName)) {
            this.warning(`entityTableName is missing in .jbooter/${ this.name }.json, using entity name as fallback`);
        ...
        validateFile: function() {
            ...
            for (var idx in this.fields) {
                ...
                if (_.isUndefined(field.fieldName)) {
                    this.error(chalk.red(`fieldName is missing in .jbooter/${ this.name }.json for field ${ JSON.stringify(field, null, 4) }`));
                }

                if (_.isUndefined(field.fieldType)) {
                    this.error(chalk.red(`fieldType is missing in .jbooter/${ this.name }.json for field ${ JSON.stringify(field, null, 4) }`));
                }

                if (!_.isUndefined(field.fieldValidateRules)) {
                    if (!_.isArray(field.fieldValidateRules)) {
                        this.error(chalk.red(`fieldValidateRules is not an array in .jbooter/${ this.name }.json for field ${ JSON.stringify(field, null, 4) }`));
                    }
                    for (var idxRules in field.fieldValidateRules) {
                        ...
                        if (!_.includes(SUPPORTED_VALIDATION_RULES, fieldValidateRule)) {
                            this.error(chalk.red(`fieldValidateRules contains unknown validation rule ${ fieldValidateRule } in .jbooter/${ this.name }.json for field ${ JSON.stringify(field, null, 4) } [supported validation rules ${ SUPPORTED_VALIDATION_RULES }]`));
                        }
                    }
                    if (_.includes(field.fieldValidateRules, 'max') && _.isUndefined(field.fieldValidateRulesMax)) {
                        this.error(chalk.red(`fieldValidateRulesMax is missing in .jbooter/${ this.name }.json for field ${ JSON.stringify(field, null, 4) }`));
                    }
                    if (_.includes(field.fieldValidateRules, 'min') && _.isUndefined(field.fieldValidateRulesMin)) {
                        this.error(chalk.red(`fieldValidateRulesMin is missing in .jbooter/${ this.name }.json for field ${ JSON.stringify(field, null, 4) }`));
                    }
                    if (_.includes(field.fieldValidateRules, 'maxlength') && _.isUndefined(field.fieldValidateRulesMaxlength)) {
                        this.error(chalk.red(`fieldValidateRulesMaxlength is missing in .jbooter/${ this.name }.json for field ${ JSON.stringify(field, null, 4) }`));
                    }
                    if (_.includes(field.fieldValidateRules, 'minlength') && _.isUndefined(field.fieldValidateRulesMinlength)) {
                        this.error(chalk.red(`fieldValidateRulesMinlength is missing in .jbooter/${ this.name }.json for field ${ JSON.stringify(field, null, 4) }`));
                    }
                    if (_.includes(field.fieldValidateRules, 'maxbytes') && _.isUndefined(field.fieldValidateRulesMaxbytes)) {
                        this.error(chalk.red(`fieldValidateRulesMaxbytes is missing in .jbooter/${ this.name }.json for field ${ JSON.stringify(field, null, 4) }`));
                    }
                    if (_.includes(field.fieldValidateRules, 'minbytes') && _.isUndefined(field.fieldValidateRulesMinbytes)) {
                        this.error(chalk.red(`fieldValidateRulesMinbytes is missing in .jbooter/${ this.name }.json for field ${ JSON.stringify(field, null, 4) }`));
                    }
                    if (_.includes(field.fieldValidateRules, 'pattern') && _.isUndefined(field.fieldValidateRulesPattern)) {
                        this.error(chalk.red(`fieldValidateRulesPattern is missing in .jbooter/${ this.name }.json for field ${ JSON.stringify(field, null, 4) }`));
                    }
                }
            }
            ...
            for (idx in this.relationships) {
                ...
                if (_.isUndefined(relationship.relationshipName)) {
                    ...
                    this.warning(`relationshipName is missing in .jbooter/${ this.name }.json for relationship ${ JSON.stringify(relationship, null, 4) }, using ${ relationship.otherEntityName } as fallback`);
                }

                if (_.isUndefined(relationship.otherEntityName)) {
                    this.error(chalk.red(`otherEntityName is missing in .jbooter/${ this.name }.json for relationship ${ JSON.stringify(relationship, null, 4) }`));
                }
                ...
                if (_.isUndefined(relationship.otherEntityRelationshipName)
                    ...
                    this.warning(`otherEntityRelationshipName is missing in .jbooter/${ this.name }.json for relationship ${ JSON.stringify(relationship, null, 4) }, using ${ _.lowerFirst(this.name) } as fallback`);
                }
                ...
                if (_.isUndefined(relationship.otherEntityField)
                    ...
                    this.warning(`otherEntityField is missing in .jbooter/${ this.name }.json for relationship ${ JSON.stringify(relationship, null, 4) }, using id as fallback`);
                    relationship.otherEntityField = 'id';
                }

                if (_.isUndefined(relationship.relationshipType)) {
                    this.error(chalk.red(`relationshipType is missing in .jbooter/${ this.name }.json for relationship ${ JSON.stringify(relationship, null, 4) }`));
                }
                ...
                if (_.isUndefined(relationship.ownerSide)
                    ...
                    this.error(chalk.red(`ownerSide is missing in .jbooter/${ this.name }.json for relationship ${ JSON.stringify(relationship, null, 4) }`));
                }
            }
            ...
            if (_.isUndefined(this.changelogDate)
                ...
                this.warning(`changelogDate is missing in .jbooter/${ this.name }.json, using ${ currentDate } as fallback`);
                this.changelogDate = currentDate;
            }
            if (_.isUndefined(this.dto)) {
                this.warning(`dto is missing in .jbooter/${ this.name }.json, using no as fallback`);
                this.dto = 'no';
            }
            if (_.isUndefined(this.service)) {
                this.warning(`service is missing in .jbooter/${ this.name }.json, using no as fallback`);
                this.service = 'no';
            }
            if (_.isUndefined(this.pagination)) {
                if (this.databaseType === 'sql' || this.databaseType === 'mongodb') {
                    this.warning(`pagination is missing in .jbooter/${ this.name }.json, using no as fallback`);
                    this.pagination = 'no';
                } else {
                    this.pagination = 'no';
                }
        ...
        afterRunHook: function () {
            try {
                ...
                if (modules.length > 0) {
                    ...
                    var entityConfig = {
                        jbooterConfigDirectory: this.jbooterConfigDirectory,
