...
module.exports = ReportGenerator.extend({
    ...
    initializing: {
        sayHello: function() {
            this.log(chalk.white('Welcome to the JBooter Information Sub-Generator\n'));
        },

        checkJBooter: function () {
            var done = this.async();
            this.log('##### **JBooter Version(s)**');
            shelljs.exec('npm list generator-jbooter', {silent:true}, function (err, stdout, stderr) {
                if (stdout) {
                    this.log('\n```\n' + stdout + '```\n');
                }
                done();
        ...
        displayConfiguration: function () {
            ...
            this.log('\n##### **JBooter configuration, a `.yo-rc.json` file generated in the root folder**\n' +
                '\n```yaml\n' + result + '\n```\n');
            done();
        },
        ...
        displayEntities: function () {
            var done = this.async();
            this.log('\n##### **Entity configuration(s) `entityName.json` files generated in the `.jbooter` directory**\n');
            shelljs.ls('.jbooter/*.json').forEach(function(file) {
