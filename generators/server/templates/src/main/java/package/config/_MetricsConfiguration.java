...
<%_ } _%>
        ...
        <%_ } _%>
        if (jBooterProperties.getMetrics().getJmx().isEnabled()) {
            log.debug("Initializing Metrics JMX reporting");
            JmxReporter jmxReporter = JmxReporter.forRegistry(metricRegistry).build();
            jmxReporter.start();
        }

        if (jBooterProperties.getMetrics().getLogs().isEnabled()) {
            ...
            final Slf4jReporter reporter = Slf4jReporter.forRegistry(metricRegistry)
                ...
                .build();
            reporter.start(jBooterProperties.getMetrics().getLogs().getReportFrequency(), TimeUnit.SECONDS);
        }
    }
    ...
    public static class GraphiteRegistry {
        ...
        @PostConstruct
        private void init() {
            if (jBooterProperties.getMetrics().getGraphite().isEnabled()) {
    ...
    @Bean
    @ConditionalOnProperty("jbooter.logging.spectator-metrics.enabled")
    @ExportMetricReader
