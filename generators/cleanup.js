...
/**
 * Removes files that where generated in previous JBooter versions and therefore need to be removed
 */
function cleanupOldFiles(generator, javaDir, testDir) {
    if (generator.isJbooterVersionLessThan('3.2.0')) {
...
/**
 * Removes server files that where generated in previous JBooter versions and therefore need to be removed
 */
function cleanupOldServerFiles(generator, javaDir, testDir) {
    if (generator.isJbooterVersionLessThan('3.5.0')) {
    ...
    if (generator.isJbooterVersionLessThan('3.6.0')) {
        generator.removeFile(javaDir + 'config/HerokuDatabaseConfiguration.java');
    }
    if (generator.isJbooterVersionLessThan('3.8.1')) {
        generator.removeFile(javaDir + 'config/JacksonConfiguration.java');
    }
    if (generator.isJbooterVersionLessThan('3.10.0')) {
    ...
    if (generator.isJbooterVersionLessThan('3.11.0')) {
        generator.removeFile(CLIENT_MAIN_SRC_DIR + 'app/layouts/navbar/active-link.directive.js');
    }
    if (generator.isJbooterVersionLessThan('3.12.0')) {
