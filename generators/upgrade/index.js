...
const GENERATOR_JBOOTER = 'generator-jbooter',
    UPGRADE_BRANCH = 'jbooter_upgrade',
...
module.exports = UpgradeGenerator.extend({
    ...
    initializing: {
        displayLogo: function () {
            this.log(chalk.green('Welcome to the JBooter Upgrade Sub-Generator '));
            this.log(chalk.green('This will upgrade your current application codebase to the latest JBooter version'));
        },

        getCurrentJHVersion: function () {
            this.currentVersion = this.config.get('jbooterVersion');
        }
    },
    ...
    _cleanUp: function() {
        shelljs.ls('-A').forEach( file => {
            if(['.yo-rc.json', '.jbooter', 'node_modules', '.git'].indexOf(file) === -1) {
                shelljs.rm('-rf', file);
            }
        });
    ...
    _generate: function(version, callback) {
        this.log('Regenerating app with jbooter ' + version + '...');
        shelljs.exec('yo jbooter --with-entities --force --skip-install', {silent:false}, function (code, msg, err) {
            if (code === 0) this.log(chalk.green('Successfully regenerated app with jbooter ' + version));
    ...
    _regenerate: function(version, callback) {
        this._generate(version, function() {
            this._gitCommitAll('Generated with JBooter ' + version, function() {
                callback();
            }.bind(this));
        }.bind(this));
    },
    ...
    configuring: {
        ...
        checkLatestVersion: function() {
            this.log('Looking for latest ' + GENERATOR_JBOOTER + ' version...');
            var done = this.async();
            shelljs.exec('npm show ' + GENERATOR_JBOOTER + ' version', {silent:true}, function (code, msg, err) {
                this.latestVersion = msg.replace('\n','');
                if (semver.lt(this.currentVersion, this.latestVersion)) {
                    this.log(chalk.green('New ' + GENERATOR_JBOOTER + ' version found: ' + this.latestVersion));
                } else if (this.force) {
        ...
        prepareUpgradeBranch: function() {
            ...
            var installJbooterLocally = function (version, callback) {
                shelljs.exec('npm ll -p --depth=0 ' + GENERATOR_JBOOTER, { silent: true }, function (code, msg, err) {
                    if (code !== 0 || msg.split('@')[1].split(':')[0] !== this.currentVersion) {
                        this.log('Installing JBooter ' + version + ' locally');
                        shelljs.exec('npm install ' + GENERATOR_JBOOTER + '@' + version, { silent: true }, function (code, msg, err) {
                            if (code === 0) this.log(chalk.green('Installed ' + GENERATOR_JBOOTER + '@' + version));
                            else this.error('Something went wrong while installing the JBooter generator! ' + msg + ' ' + err);
                            callback();
                        }.bind(this));
                    } else {
                        this.log('Using already installed JBooter ' + version);
                        callback();
                    }
                }.bind(this));
            }.bind(this);
            ...
            var regenerate = function() {
                this._cleanUp();
                installJbooterLocally(this.currentVersion, function() {
    ...
    default: {
        ...
        updateJbooter: function() {
            this.log(chalk.yellow('Updating ' + GENERATOR_JBOOTER + '. This might take some time...'));
            var done = this.async();
            shelljs.exec('npm install ' + GENERATOR_JBOOTER, {silent:true}, function (code, msg, err) {
                if (code === 0) this.log(chalk.green('Updated ' + GENERATOR_JBOOTER + ' to version ' + this.latestVersion));
