...
const JBOOTER_CONFIG_DIR = '.jbooter';
const MODULES_HOOK_FILE = JBOOTER_CONFIG_DIR + '/modules/jhi-hooks.json';
const GENERATOR_JBOOTER = 'generator-jbooter';
...
Generator.prototype.addElementToMenu = function (routerName, glyphiconName, enableTranslation) {
    try {
        ...
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-add-element-to-menu',
        ...
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + routerName + ' ' + chalk.yellow('not added to menu.\n'));
    }
};
...
Generator.prototype.addElementToAdminMenu = function (routerName, glyphiconName, enableTranslation) {
    try {
        ...
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-add-element-to-admin-menu',
            splicable: [`<li ui-sref-active="active" >
                            <a ui-sref="${routerName}" ng-click="vm.collapseNavbar()">
                                <span class="glyphicon glyphicon-${glyphiconName}"></span>&nbsp;
        ...
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + routerName + ' ' + chalk.yellow('not added to admin menu.\n'));
    }
};
...
Generator.prototype.addEntityToMenu = function (routerName, enableTranslation) {
    try {
        ...
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-add-entity-to-menu',
            splicable: [`<li ui-sref-active="active">
                            <a ui-sref="${routerName}" ng-click="vm.collapseNavbar()">
...
Generator.prototype.addElementTranslationKey = function (key, value, language) {
    ...
    try {
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-menu-add-element',
            splicable: [
                `"${key}": "${_.startCase(value)}",`
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + language + chalk.yellow(' not added as a new entity in the menu.\n'));
    }
};
...
Generator.prototype.addAdminElementTranslationKey = function (key, value, language) {
    ...
    try {
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-menu-add-admin-element',
            splicable: [
                `"${key}": "${_.startCase(value)}",`
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + language + chalk.yellow(' not added as a new entry in the admin menu.\n'));
    }
};
...
Generator.prototype.addEntityTranslationKey = function (key, value, language) {
    ...
    try {
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-menu-add-entry',
            splicable: [
                `"${key}": "${_.startCase(value)}",`
            ]
        }, this);
...
Generator.prototype.addGlobalTranslationKey = function (key, value, language) {
    var fullPath = CLIENT_MAIN_SRC_DIR + 'i18n/' + language + '/global.json';
    try {
        jbooterUtils.rewriteJSONFile(fullPath, function (jsonObj) {
            jsonObj[key] = value;
        }, this);
    } catch (e) {
...
/**
 * get all the languages supported by JBooter
 */
...
/**
 * check if a language is supported by JBooter
...
/**
 * get all the languages options supported by JBooter
 */
...
Generator.prototype.addSocialConfiguration = function (name, clientId, clientSecret, comment) {
    ...
    try {
        ...
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-add-social-configuration',
            splicable: [
                config
            ]
        }, this);
...
Generator.prototype.addAngularJsModule = function (moduleName) {
    ...
    try {
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-angularjs-add-module',
            splicable: [
                `'${moduleName}',`
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + moduleName + chalk.yellow(' not added to JBooter app.\n'));
    }
};
...
Generator.prototype.addAngularJsInterceptor = function (interceptorName) {
    ...
    try {
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-angularjs-add-interceptor',
            splicable: [
                `$httpProvider.interceptors.push('${interceptorName}');`
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Interceptor not added to JBooter app.\n'));
    }
};
...
Generator.prototype.addEntryToEhcache = function (entry) {
    try {
        ...
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-ehcache-add-entry',
...
Generator.prototype.addChangelogToLiquibase = function (changelogName) {
    this.addLiquibaseChangelogToMaster(changelogName, 'jbooter-needle-liquibase-add-changelog');
};
...
Generator.prototype.addConstraintsChangelogToLiquibase = function (changelogName) {
    this.addLiquibaseChangelogToMaster(changelogName, 'jbooter-needle-liquibase-add-constraints-changelog');
};
...
Generator.prototype.addLiquibaseChangelogToMaster = function (changelogName, needle) {
    try {
        ...
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + changelogName + '.xml ' + chalk.yellow('not added.\n'));
    }
};
...
Generator.prototype.addColumnToLiquibaseEntityChangeset = function (filePath, content) {
    try {
        jbooterUtils.rewriteFile({
            file: filePath,
            needle: 'jbooter-needle-liquibase-add-column',
            splicable: [
                content
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + filePath + chalk.yellow(' or missing required jbooter-needle. Column not added.\n') + e);
    }
};
...
Generator.prototype.addSocialButton = function (isUseSass, socialName, socialParameter, buttonColor, buttonHoverColor) {
    ...
    try {
        ...
        jbooterUtils.rewriteFile({
            ...
            needle: 'jbooter-needle-add-social-button',
            splicable: [
                serviceCode
            ]
        }, this);
 ...
 *
 * @param {string} javaDir - default java directory of the project (JBooter var)
...
Generator.prototype.addSocialConnectionFactory = function (javaDir, importPackagePath, socialName, connectionFactoryClassName, configurationName) {
    ...
    try {
        ...
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-add-social-connection-factory-import-package',
            splicable: [
                javaImport
            ]
        }, this);
        ...
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-add-social-connection-factory',
            splicable: [
                javaCode
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Social connection ') + e + ' ' + chalk.yellow('not added.\n'));
    }
};
...
Generator.prototype.addMainCSSStyle = function (isUseSass, style, comment) {
    ...
    try {
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-css-add-main',
            splicable: [
                styleBlock
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Style not added to JBooter app.\n'));
    }
};
...
Generator.prototype.addMainSCSSStyle = function (style, comment) {
    ...
    try {
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-scss-add-main',
            splicable: [
                styleBlock
            ]
        }, this);
...
Generator.prototype.addMavenDependency = function (groupId, artifactId, version, other) {
    try {
        ...
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-maven-add-dependency',
            splicable: [
                dependency
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + 'maven dependency (groupId: ' + groupId + ', artifactId:' + artifactId + ', version:' + version + ')' + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.addMavenPlugin = function (groupId, artifactId, version, other) {
    try {
        ...
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-maven-add-plugin',
            splicable: [
                plugin
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + 'maven plugin (groupId: ' + groupId + ', artifactId:' + artifactId + ', version:' + version + ')' + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.addGradlePlugin = function (group, name, version) {
    try {
        ...
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-gradle-buildscript-dependency',
            splicable: [
                `classpath '${group}:${name}:${version}'`
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + 'classpath: ' + group + ':' + name + ':' + version + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.addGradleDependency = function (scope, group, name, version) {
    try {
        ...
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-gradle-dependency',
            splicable: [
                `${scope} '${group}:${name}:${version}'`
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + group + ':' + name + ':' + version + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.applyFromGradleScript = function (name) {
    try {
        ...
        jbooterUtils.rewriteFile({
            file: fullPath,
            needle: 'jbooter-needle-gradle-apply-from',
            splicable: [
                `apply from: '${name}.gradle'`
            ]
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + name + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.copyTemplate = function (source, dest, action, generator, opt, template) {
    ...
    case 'stripHtml' :
        ...
        jbooterUtils.copyWebResource(source, dest, regex, 'html', _this, _opt, template);
        break;
    case 'stripJs' :
        regex = /\,[\s\n ]*(resolve)\:[\s ]*[\{][\s\n ]*[a-zA-Z]+\:(\s)*\[[ \'a-zA-Z0-9\$\,\(\)\{\}\n\.\<\%\=\-\>\;\s]*\}\][\s\n ]*\}/g;
        //looks for something like mainTranslatePartialLoader: [*]
        jbooterUtils.copyWebResource(source, dest, regex, 'js', _this, _opt, template);
        break;
    case 'copy' :
...
/**
 * Register a module configuration to .jbooter/modules/jhi-hooks.json
 *
 * @param {string} npmPackageName - npm package name of the generator
 * @param {string} hookFor - from which JBooter generator this should be hooked ( 'entity' or 'app')
...
Generator.prototype.registerModule = function (npmPackageName, hookFor, hookType, callbackSubGenerator, description) {
    try {
        var modules;
        var error, duplicate;
        var moduleName = _.startCase(npmPackageName.replace(GENERATOR_JBOOTER + '-', ''));
        ...
        var moduleConfig = {
            ...
            description: description ? description : 'A JBooter module to generate ' + moduleName,
            hookFor: hookFor,
            hookType: hookType,
        ...
        try {
            ...
            error = true;
            this.log(chalk.red('The JBooter module configuration file could not be read!'));
        }
        if (!error && !duplicate) {
        ...
        this.log('\n' + chalk.bold.red('Could not add jbooter module configuration'));
    }
};
...
Generator.prototype.updateEntityConfig = function (file, key, value) {
    ...
    try {
        ...
        this.log(chalk.red('The JBooter entity configuration file could not be read!') + err);
    }
...
Generator.prototype.getEntityProperty = function (file, key) {
    ...
    try {
        var entityJson = this.fs.readJSON(path.join(JBOOTER_CONFIG_DIR, _.upperFirst(file) + '.json'));
...
Generator.prototype.getExistingEntities = function () {
    ...
    if (shelljs.test('-d', JBOOTER_CONFIG_DIR)) {
        shelljs.ls(path.join(JBOOTER_CONFIG_DIR, '*.json')).forEach(function (file) {
...
/**
 * Check if the JBooter version used to generate an existing project is less than the passed version argument
 *
 * @param {string} version - A valid semver version string
 */
Generator.prototype.isJbooterVersionLessThan = function (version) {
    var jbooterVersion = this.config.get('jbooterVersion');
    if (!jbooterVersion) {
        return true;
    }
    return semver.lt(jbooterVersion, version);
};
...
/**
 * get a table name in JBooter preferred style.
 *
...
/**
 * get a table column name in JBooter preferred style.
 *
...
/**
 * get a table column names plural form in JBooter preferred style.
 *
 * @param {string} value - table column name string
 */
...
/**
 * get a table name for joined tables in JBooter preferred style.
 *
...
/**
 * get a constraint name for tables in JBooter preferred style.
 *
 * @param {string} entityName - name of the entity
 * @param {string} relationshipName - name of the related entity
 * @param {string} prodDatabaseType - database type
 ...
 */
Generator.prototype.printJBooterLogo = function () {
    this.log(' \n' +
    ...
    this.log(chalk.white.bold('                            http://jbooter.github.io\n'));
...
/**
 * Checks if there is a newer JBooter version available.
 */
Generator.prototype.checkForNewVersion = function () {
    try {
        shelljs.exec('npm show ' + GENERATOR_JBOOTER + ' version', {silent:true}, function (code, stdout, stderr) {
            if (!stderr && semver.lt(packagejs.version, stdout)) {
                this.log(
                    ...
                    chalk.yellow('  JBooter update available: ') + chalk.green.bold(stdout.replace('\n','')) + chalk.gray(' (current: ' + packagejs.version + ')') + '\n' +
                    chalk.yellow('  Run ' + chalk.magenta('npm install -g ' + GENERATOR_JBOOTER ) + ' to update.\n') +
                    chalk.yellow(' ______________________________________________________________________________\n')
                );
            }
        }.bind(this));
...
Generator.prototype.composeLanguagesSub = function (generator, configOptions, type) {
    if (generator.enableTranslation) {
        ...
        generator.composeWith('jbooter:languages', {
            options: {
                'skip-install': true,
...
Generator.prototype.updateLanguagesInLanguageConstant = function (languages) {
    ...
    try {
        ...
        content +=
            '            // jbooter-needle-i18n-language-constant - JBooter will add/remove languages in this array\n' +
            '        ]';

        jbooterUtils.replaceContent({
            file: fullPath,
            pattern: /\.constant.*LANGUAGES.*\[([^\]]*jbooter-needle-i18n-language-constant[^\]]*)\]/g,
            content: content
        }, this);
    } catch (e) {
        this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. LANGUAGE constant not updated with languages: ') + languages + chalk.yellow(' since block was not found. Check if you have enabled translation support.\n'));
    }
};
...
Generator.prototype.getDefaultAppName = function () {
    return (/^[a-zA-Z0-9_]+$/.test(path.basename(process.cwd()))) ? path.basename(process.cwd()) : 'jbooter';
};

Generator.prototype.formatAsClassJavadoc = function (text) {
    return jbooterUtils.getJavadoc(text, 0);
};

Generator.prototype.formatAsFieldJavadoc = function (text) {
    return jbooterUtils.getJavadoc(text, 4);
};
...
/**
 * get hibernate SnakeCase in JBooter preferred style.
 *
