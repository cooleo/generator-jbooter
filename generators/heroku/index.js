...
module.exports = HerokuGenerator.extend({
    ...
    default: {
        ...
        configureJBooterRegistry: function() {
            if (this.abort || this.herokuAppExists) return;
            var done = this.async();
            ...
            if (this.applicationType === 'microservice' || this.applicationType === 'gateway') {
                var prompts = [
                    {
                        type: 'input',
                        name: 'herokuJBooterRegistry',
                        message: 'What is the URL of your JBooter Registry?'
                    }];

                this.log('');
                this.prompt(prompts).then(function (props) {
                    var configSetCmd = 'heroku config:set ' + 'JBOOTER_REGISTRY_URL=' + props.herokuJBooterRegistry + ' --app ' + this.herokuAppName;
